...
def cargar_csv():
    if 'archivo' not in request.files:
        return jsonify({'mensaje': 'No se proporcionó ningún archivo en el Blueprint Funciones'}), 400

    archivo = request.files['archivo']

    if archivo.filename == '':
        return jsonify({'mensaje': 'No se seleccionó ningún archivo en el Blueprint Funciones'}), 400
    ...
    if archivo and archivo.filename.endswith('.csv'):
        try:
            ...
            return jsonify({'mensaje': f'Error al procesar el archivo CSV en el Blueprint Funciones: {str(e)}'}), 500

    return jsonify({'mensaje': 'El archivo no tiene el formato CSV correcto en el Blueprint Funciones'}), 400